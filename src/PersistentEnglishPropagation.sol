...
abstract contract PersistentEnglishPropagation is Ownable, ERC721 {
    /*//////////////////////////////////////////////////////////////
                                 EVENTS
    //////////////////////////////////////////////////////////////*/
    ...
    function bid() public payable {
        ...
        // We note that we do not take any bids that are made after the
        // current auction round has started.
        _backfillBids();
    ...
    ///@notice Gets the total rounds that have passed
    function getRoundsPassed() public view returns (uint256) {
        return
            (uint256(block.timestamp) - uint256(auctionStartTime)) /
            uint256(timeBetweenSells);
    }
    ...
    function _addBid(address bidder, uint256 bidAmount) private {
        bids.push(Bid(bidder, bidAmount, block.timestamp));
    }

    ///@notice Contains bids from previous rounds?
    function _containsBidsFromPreviousRounds() private view returns (bool) {
        uint256 previousRoundEndTime = getRoundsPassed() *
            timeBetweenSells +
            auctionStartTime;

        for (uint256 i = 0; i < bids.length; i++) {
            if (bids[i].timestamp < previousRoundEndTime) {
                return true;
            }
        }
        return false;
    ...
    function _getWinningIndex() private view returns (uint256) {
        ...
        // We only want to consider bids from previous rounds i.e
        // bids with a timestamp less than the previous round's end time
        uint256 previousRoundEndTime = ((uint256(block.timestamp) -
            uint256(auctionStartTime)) / uint256(timeBetweenSells)) *
            timeBetweenSells +
            auctionStartTime;

        for (uint256 i = 0; i < bids.length; i++) {
            if (
                bids[i].timestamp < previousRoundEndTime &&
                bids[i].amount > highestAmount
            ) {
                winningIndex = i;
    ...
    ///@notice Backfill bids from previous rounds
    function _backfillBids() private {
        // If there are passed auction rounds without any winners and there are
        // bids from previous rounds, we backfill
        while (
            remainingToSell > 0 &&
            getRoundsPassed() > (totalToSell - remainingToSell) &&
            _containsBidsFromPreviousRounds()
        ) {
            _acceptTopBid();
        }
    }
